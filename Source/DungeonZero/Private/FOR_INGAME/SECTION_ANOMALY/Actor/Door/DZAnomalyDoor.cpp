...
void ADZAnomalyDoor::EndPlay(const EEndPlayReason::Type EndPlayReason)
{
	// 메시지 구독 해제
...
void ADZAnomalyDoor::BeginPlay()
{
	...
	// 리셋 메시지 구독
	UGameplayMessageSubsystem& MessageSubsystem = UGameplayMessageSubsystem::Get(this);
	TimeResetListenerHandle = MessageSubsystem.RegisterListener<FDZDoorMSG>(DZ::DoorMSG::DZ_DOOR_DOORRESET, this, &ADZAnomalyDoor::OnDoorResetReceived);
}
...
void ADZAnomalyDoor::OnDoorResetReceived(FGameplayTag Channel, const FDZDoorMSG& Payload)
{
	// onRep 호출 (클라 동기화)
